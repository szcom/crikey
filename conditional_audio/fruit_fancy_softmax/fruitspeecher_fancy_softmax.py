...
from kdllib import make_biases, theano_one_hot, categorical_crossentropy, softmax
from kdllib import sample_softmax
...
if __name__ == "__main__":
    ...
    # n_proj + 1 for concatenated out and prev
    v_cell1 = GRU(n_proj + 1, n_proj, random_state)
    params += v_cell1.get_params()

    inp_to_v_h1 = GRUFork(n_proj + 1, n_proj, random_state)
    params += inp_to_v_h1.get_params()

    prev_proj, = make_weights(input_dim, [n_proj], random_state)
    b_prev_proj, = make_biases([n_proj])
    params += [prev_proj, b_prev_proj]

    softmax_proj, = make_weights(n_proj, [n_softmax], random_state)
    b_softmax_proj, = make_biases([n_softmax])
    ...
    def sample_step(x_tm1, h1_tm1, h2_tm1, h3_tm1, k_tm1, w_tm1, ctx):
        ...
        prev = x_tm1.dot(prev_proj) + b_prev_proj
        theano.printing.Print("out_t.shape")(out_t.shape)
        theano.printing.Print("x_tm1.shape")(x_tm1.shape)
        theano.printing.Print("prev.shape")(prev.shape)

        def sample_out_step(o_tm1, x_tm1, v_h1_tm1, prev):
            j_tm1 = tensor.concatenate((o_tm1, prev), axis=-1)
            vinp_h1_t, vgate_h1_t = inp_to_v_h1.proj(j_tm1)
            v_h1_t = v_cell1.step(vinp_h1_t, vgate_h1_t, v_h1_tm1)
            pred_t = v_h1_t.dot(softmax_proj) + b_softmax_proj
            theano.printing.Print("pred_t.shape")(pred_t.shape)
            pred_t = softmax(pred_t * (1. + softmax_bias_sym))
            pred_t = sample_softmax(pred_t, srng)
            pred_t = pred_t.dimshuffle(0, 'x')
            theano.printing.Print("pred_t.shape")(pred_t.shape)
            return pred_t, v_h1_t

        out_t = out_t.dimshuffle(1, 0, 'x')
        init_samp_out = tensor.zeros((out_t.shape[1], 1))
        init_corr_out_t = tensor.zeros((out_t.shape[1], n_proj))
        r, isupdates = theano.scan(
            fn=sample_out_step,
            sequences=[out_t],
            outputs_info=[init_samp_out, init_corr_out_t],
            non_sequences=[prev],
        )
        samp_t = r[0]
        x_t = samp_t.dimshuffle(2, 1, 0)[0]
    ...
    shuff_inpt = inpt
    shuff_inpt = shuff_inpt.reshape((shuff_inpt_shapes[0] * shuff_inpt_shapes[1], -1))
    prev_inpt_proj = shuff_inpt.dot(prev_proj) + b_prev_proj
    theano.printing.Print("shuff_inpt.shape")(shuff_inpt.shape)
    theano.printing.Print("prev_inpt_proj.shape")(prev_inpt_proj.shape)
    theano.printing.Print("inpt.shape")(inpt.shape)
    theano.printing.Print("outs.shape")(outs.shape)

    def out_step(o_tm1, v_h1_tm1, prev):
        j_tm1 = tensor.concatenate((o_tm1, prev), axis=-1)
        vinp_h1_t, vgate_h1_t = inp_to_v_h1.proj(j_tm1)
        v_h1_t = v_cell1.step(vinp_h1_t, vgate_h1_t, v_h1_tm1)
        return v_h1_t

    init_corr_outs = tensor.zeros((outs.shape[1], n_proj))
    corr_outs, updates = theano.scan(
        fn=out_step,
        sequences=[outs],
        outputs_info=[init_corr_outs],
        non_sequences=[prev_inpt_proj],
    )
    corr_outs = corr_outs.dimshuffle(1, 0, 2)
    theano.printing.Print("corr_outs.shape")(corr_outs.shape)
    shp = corr_outs.shape
    corr_outs = corr_outs.reshape((-1, shp[-1]))
    preds = corr_outs.dot(softmax_proj) + b_softmax_proj
    preds = softmax(preds * (1. + softmax_bias_sym))
    # 3D
    preds = preds.reshape((shp[0], shp[1], -1))
    # 4D time, mb, n_features, n_softmax
    preds = preds.reshape((orig_shapes[0], orig_shapes[1], shp[1], -1))
    theano.printing.Print("preds.shape")(preds.shape)
    target = theano_one_hot(target)
    ...
    cost = cost.sum() / (cut_len * target.shape[1])
    ...
    def _loop(function, itr):
        ...
        for n in range(n_cuts):
            ...
            softmax_bias = 0.  # No bias in training
            rval = function(X_mb[start:stop],
                            X_mb_mask[start:stop],
                            c_mb, c_mb_mask,
                            prev_h1, prev_h2, prev_h3, prev_kappa, prev_w, softmax_bias)
            current_cost = rval[0]
